...
// Paths to exclude from protection (static assets, API, etc.)
...
export function middleware(req: NextRequest) {
  ...
  // If cookie exists and still fresh, allow
  if (cookie) {
  ...
  // Not authorized or expired → show a small inline login page.
...
<style>
  ...
  body{display:flex;align-items:center;justify-content:center;background:#0f1724;font-family:system-ui, -apple-system, Segoe UI, Roboto, Arial;color:#e6eef8}
  ...
  input{width:100%;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:#e6eef8;font-size:14px;outline:none}
  ...
  .err{min-height:18px;margin-top:8px;font-size:12px;color:#ef4444}
</style>
</head>
<body>
...
export const config = {
  matcher: ["/((?!_next).*)"], // run on all paths (assets bypassed in isBypassed)
};
